...
pub fn add_default_groups(
    ...
    // Skip if either vector is empty (happens when batch processing is enabled)
    if vec_groups.is_empty() || vec_computers.is_empty() {
        return Ok(());
    }
...
pub fn add_default_users(
    ...
    // Skip if vec_users is empty (happens when batch processing is enabled)
...
pub fn add_childobjects_members<T: LdapObject>(
    ...
    // Skip if vec_replaced is empty (happens when batch processing is enabled)
    if vec_replaced.is_empty() {
        return Ok(());
    }
...
pub fn add_affected_computers(
    ...
    // Skip if vec_domains is empty (happens when batch processing is enabled)
    if vec_domains.is_empty() {
        return Ok(());
    }

    // Create a vector to store all computers
    let mut vec_affected_computers: Vec<Member> = Vec::new();

    // Filter only "Computer" SIDs and map them to Member objects
    vec_affected_computers = sid_type
        .iter()
...
pub fn add_affected_computers_for_ou(
    ...
    // Skip if vec_ous is empty (happens when batch processing is enabled)
    if vec_ous.is_empty() {
        return Ok(());
    }

    // Create a filtered map of computer DNs to SIDs
    let computer_dn_sids: HashMap<String, String> = dn_sid
        .iter()
        .filter(|(_, sid)| sid_type.get(*sid).map(|t| t == "Computer").unwrap_or(false))
        .map(|(dn, sid)| (dn.clone(), sid.clone()))
        .collect();
    ...
    for ou in vec_ous.iter_mut() {
        if let Some(ou_dn) = ou_dn_map.get(ou.get_object_identifier()) {
            let vec_affected_computers: Vec<Member> = computer_dn_sids
                .iter()
                .filter_map(|(dn, sid)| {
...
pub fn replace_fqdn_by_sid<T: LdapObject>(
    ...
    // Skip if vector is empty (happens when batch processing is enabled)
    if vec_src.is_empty() {
        return Ok(());
    }
...
pub fn add_trustdomain(
    ...
    // Skip if either vector is empty (happens when batch processing is enabled)
    if vec_domains.is_empty() || vec_trusts.is_empty() {
        return Ok(());
    }

    // Only process if we have actual trust domains
...
pub fn add_type_for_ace<T: LdapObject>(
    ...
    // Skip if object vector is empty (happens when batch processing is enabled)
    if object.is_empty() {
        return Ok(());
    }
...
pub fn add_type_for_allowtedtoact(
    ...
    // Skip if computer vector is empty (happens when batch processing is enabled)
